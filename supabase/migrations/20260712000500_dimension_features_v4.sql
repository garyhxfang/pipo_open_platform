...
begin
  ...
  if schema_version not in ('1', '2', '3', '4') then
  ...
  if schema_version = '4' and (
    ...
    raise exception 'Invalid capability configuration v4 payload';
  end if;
...
comment on function public.publish_config(text, jsonb) is
  'Publishes schema v4 capability snapshots where merchant, product, environment, and integration options are configurable capability features.';
