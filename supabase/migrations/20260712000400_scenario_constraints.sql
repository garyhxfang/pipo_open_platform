...
begin
  ...
  if schema_version not in ('1', '2', '3') then
    raise exception 'Unsupported configuration schema version';
  end if;

  if schema_version = '3' and (
    jsonb_typeof(p_payload -> 'dimensions') is distinct from 'array'
    or jsonb_typeof(p_payload -> 'scenarioConstraints') is distinct from 'array'
    or jsonb_typeof(p_payload -> 'capabilities') is distinct from 'array'
    or jsonb_typeof(p_payload -> 'scenarioRules') is distinct from 'array'
    or jsonb_typeof(p_payload -> 'marketRules') is distinct from 'array'
    or jsonb_typeof(p_payload -> 'marketPairExceptions') is distinct from 'array'
    or jsonb_typeof(p_payload -> 'conflicts') is distinct from 'array'
  ) then
    raise exception 'Invalid capability configuration v3 payload';
  end if;
...
comment on function public.publish_config(text, jsonb) is
  'Publishes immutable capability configuration snapshots, including global scenario constraints and market-separated capability rules.';
